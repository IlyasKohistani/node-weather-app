...
const forecast = (latitude, longitude, callback) => {
  const url = `http://api.weatherstack.com/current?access_key=${process.env.WEATHERSTACK_ACCESS_KEY}&query=${longitude},${latitude}`;
  request({ url, json: true }, (error, { body } = {}) => {
    if (error)
      callback({ error: "Unable to connect to weather services!" }, undefined);
    else if (body.error)
      callback(
        { error: "Unable to find the location! Please try again." },
        undefined
      );
    else {
